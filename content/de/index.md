...
- **Linux** — ein nativer GTK-3-Desktop-Client (Application-ID `com.ahmadre.hinata`), installiert mit `snap install hinata` — ein strikt isoliertes Snap für amd64 und arm64 im [Snap Store](https://snapcraft.io/hinata); Flatpak- und AppImage-Rezepte liegen weiterhin im Repository, wer lieber selbst baut ([Die Apps](/de/clients.html#hinata-unter-linux) hat die Details, inklusive der zwei Berechtigungen, die Snap dich verbinden lässt). Ein `hinata://`-Link — ein SSO-Rücksprung, eine Einladung, ein Passwort-Reset — erreicht das bereits geöffnete Fenster, denn die App registriert den Scheme-Handler und läuft als Einzelinstanz.
