...
- **Linux** — ein nativer GTK-3-Desktop-Client (Application-ID `com.ahmadre.hinata`), als strikt isoliertes Snap für amd64 und arm64 gebaut und für den Snap Store bestimmt, wo aber noch keine Revision auf einem Kanal liegt — heute baust du ihn selbst, aus den Flatpak- oder AppImage-Rezepten im Repository ([Die Apps](/de/clients.html#hinata-unter-linux) nennt den aktuellen Installationsstand). Ein `hinata://`-Link — ein SSO-Rücksprung, eine Einladung, ein Passwort-Reset — erreicht das bereits geöffnete Fenster, denn die App registriert den Scheme-Handler und läuft als Einzelinstanz.
