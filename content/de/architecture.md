...
- **App (Flutter)** — eine Codebasis, sechs Ziele: Android, iOS, Web, macOS, Windows und Linux (eine native GTK-3-Desktop-App mit der Application-ID `com.ahmadre.hinata`). State wird mit bloc/cubit verwaltet, Routing mit go_router, Lokalisierung mit i18next (Englisch + Deutsch) und Networking über **dio** innerhalb eines `ApiClient`. Diagramme werden mit fl_chart gezeichnet.
...
!!! note "Linux bekommt die Benachrichtigungen, aber kein Push"
    Push braucht einen Zustelldienst des Betriebssystems, und unter Linux gibt es keinen, bei dem sich die App registrieren könnte — `firebase_messaging` hat keine Linux-Implementierung. Ein Linux-Client registriert deshalb nie ein Push-Token, und das Gateway hat nichts, wohin es zustellen könnte. Dieselben Ereignisse erreichen den Nutzer trotzdem: im Benachrichtigungscenter der App und per E-Mail. Der Push-Schalter in den Kontoeinstellungen bleibt aktiv, denn diese Einstellung gehört zum Konto und steuert weiterhin das Telefon derselben Person.
