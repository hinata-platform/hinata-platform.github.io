...
- **Linux** — a native GTK 3 desktop client (application id `com.ahmadre.hinata`), built as a strictly confined snap for amd64 and arm64 and headed for the Snap Store, though no revision is on a channel yet — today you build it, from the Flatpak or AppImage recipes in the repository ([The apps](/en/clients.html#hinata-on-linux) has the current install status). A `hinata://` link — an SSO callback, an invite, a password reset — reaches the window that is already open, because the app registers the scheme handler and runs as a single instance.
