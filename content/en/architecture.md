...
- **App (Flutter)** — one codebase, six targets: Android, iOS, the web, macOS, Windows and Linux (a native GTK 3 desktop application, application id `com.ahmadre.hinata`). State is managed with bloc/cubit, routing with go_router, localization with i18next (English + German), and networking through **dio** inside an `ApiClient`. Charts are drawn with fl_chart.
...
!!! note "Linux gets the notifications, not the push"
    Push needs a delivery service the operating system provides, and Linux has none to register with — `firebase_messaging` has no Linux implementation. A Linux client therefore never registers a push token, and the gateway has nothing to deliver to it. The same events still reach the user: in the app's own notification centre, and by e-mail. The push switch in account settings stays live because that preference belongs to the account and still governs the same person's phone.
